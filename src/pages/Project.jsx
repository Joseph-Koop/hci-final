...
function Project({ equipmentList, employeeList, projectsList, onDeleteProject, onAddLog, onEditProjectMaterials, onEditProjectCompletion, onEditLog, onDeleteLog, onEditProject }) {
  const { id } = useParams();
  ...
  const handleDeleteLog = (e) => {
    e.preventDefault();
    onDeleteLog(project.id, currentLog.index);
  ...
  const handleEditProject = (e) => {
    ...
    onEditProject({
      ...
      expected_completion: editProjectForm.expected_completion,
      locationX: parseFloat(editProjectForm.locationX),
      locationY: parseFloat(editProjectForm.locationY)
    });
  ...
  return (
    <div className="bg-(--light2) dark:bg-(--dark1) w-full min-h-screen text-gray-800 dark:text-white p-6">
          ...
          <div className="space-y-3">
            {project.materials.length > 0 ? (
              project.materials.map((material, index) => (
                <div key={index} className="flex justify-between items-center px-4 py-2 rounded-lg border bg-orange-50 dark:bg-orange-500/30 border-orange-200 dark:border-orange-500">
                ...
                <div key={log.id || index} className="flex justify-between items-center px-4 py-2 rounded-lg border bg-orange-50 dark:bg-orange-500/30 border-orange-200 dark:border-orange-500">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => {
                        setCurrentLog({ ...log, index });
                        setShowDeleteLogModal(true);   //handleDeleteLog(log.id)}
                      }}
      ...
      {/* Footer */}
      <div className="bg-(--light2) dark:bg-(--dark1) flex justify-end items-center mb-6 mt-6">
          <button
            onClick={() => {
    ...
    {/* Edit Project Modal */}
      {showEditProjectModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-(--dark2) rounded-xl w-full max-w-md p-6 shadow-lg border border-gray-200 dark:border-gray-700">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-semibold text-gray-800 dark:text-white">Edit Project</h3>
              <button
            ...
            <form onSubmit={handleEditProject} className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="date"
                  value={editProjectForm.expected_completion}
                  onChange={(e) => setEditProjectForm({ ...editProjectForm, expected_completion: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-(--main1) dark:bg-(--dark2) dark:text-white"
                  required
                />
              </div>
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <input
                    ...
                    value={editProjectForm.locationX}
                    onChange={(e) => setEditProjectForm({ ...editProjectForm, locationX: e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-(--main1) dark:bg-(--dark2) dark:text-white"
                    required
                  />
                  <input
                    ...
                    value={editProjectForm.locationY}
                    onChange={(e) => setEditProjectForm({ ...editProjectForm, locationY: e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-(--main1) dark:bg-(--dark2) dark:text-white"
                    required
                  />
                </div>
      ...
      {showDeleteProjectModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-(--dark2) rounded-xl w-full max-w-md p-6 shadow-lg border border-gray-200 dark:border-gray-700">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-semibold text-gray-800 dark:text-white">Delete Project</h3>
              <button
